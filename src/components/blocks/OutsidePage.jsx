...
import { AppBar, darken, Fade, IconButton, lighten, Toolbar, Typography } from '@material-ui/core';
...
const useAppBarStyles = makeStyles(theme => ({
  ...
  header: {
    backgroundColor: ({ isShrinked, backgroundColor, shrinkedBackgroundColor }) => (
      isShrinked
    ...
    flexGrow: 1,
    transform: ({ isShrinked }) => `translateZ(0) translateY(${isShrinked ? -16 : 0}px)`,
    transition: ({ disableShrinking }) => (disableShrinking ? 'none' : 'all .3s cubic-bezier(0.4, 0, 0.2, 1)'),
    [theme.breakpoints.up(MIDDLE_WIDTH)]: {
  ...
  toolbar: {
    ...
    '&::before': {
      position: 'absolute',
      width: ({ scrollProgress }) => `${scrollProgress * 100}%`,
      background: theme.palette.type === 'dark' ? 'rgba(255, 255, 255, .1)' : 'rgba(0, 0, 0, .1)',
      height: '100%',
      content: '""',
  ...
  backButtonDesktop: {
    ...
    '&:hover': {
      background: isDarkTheme(theme)
        ? lighten(getSecondaryAppBarColor(theme), 0.05)
        : darken(getSecondaryAppBarColor(theme), 0.05),
    },
    [theme.breakpoints.up(MIDDLE_WIDTH)]: {
      display: 'flex',
    },
...
const UnshrinkedContentUnmemoized = ({ headerText, onBackClick, isShrinked, headerTextUpdated }) => {
  const classes = useAppBarStyles({});
...
const OutsidePage = ({ children, ...props }) => {
